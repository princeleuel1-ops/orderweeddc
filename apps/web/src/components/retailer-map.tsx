...
export default function RetailerMap({ retailers }: RetailerMapProps) {
  ...
  if (!mounted) {
    return <div className="w-full h-full bg-slate-100 animate-pulse flex items-center justify-center text-xs text-slate-400">Loading Map...</div>;
  }
  ...
  const createIcon = (name: string, isSponsored: boolean = false) => L.divIcon({
    ...
    html: `<div class="w-8 h-8 rounded-full flex items-center justify-center shadow-lg border-2 border-white ${isSponsored ? 'bg-brand-primary text-white scale-110' : 'bg-slate-900 text-white'} text-xs font-black transition-transform hover:scale-125">${name.substring(0, 1).toUpperCase()}</div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 16],
    popupAnchor: [0, -16],
  });
        ...
        <Marker key={r.id} position={[r.lat, r.lng]} icon={createIcon(r.name)}>
          <Popup className="premium-popup">
            <div className="font-sans flex flex-col items-center min-w-[140px]">
              ...
              <Link 
                href={`/retailer/${r.id}`} 
                className="text-xs bg-slate-900 hover:bg-slate-800 text-white font-bold px-4 py-1.5 rounded w-full text-center transition-colors shadow"
              >
                View Directory
              </Link>
            </div>
